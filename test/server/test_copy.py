...
class TestCopy(TestBase):
    ...
    async def test_copy(self):
        ...
        self.transport.push_write(
            b'copy1 OK [COPYUID ', (br'\d+', ), b' 101:104 101:104]'
            b' COPY completed.\r\n')
    ...
    async def test_concurrent_copy_fetch(self):
        ...
        concurrent.push_select(b'Trash', 0, 0, unseen=False, set=event1)
        concurrent.push_readline(
            b'noop1 NOOP\r\n', wait=event2)
        concurrent.push_write(
            b'* 4 EXISTS\r\n'
            b'* 4 RECENT\r\n'
            b'* 1 FETCH (FLAGS (\\Recent \\Seen))\r\n'
            b'* 2 FETCH (FLAGS (\\Answered \\Recent \\Seen))\r\n'
            b'* 3 FETCH (FLAGS (\\Flagged \\Recent \\Seen))\r\n'
